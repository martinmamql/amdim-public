...
class NCE_MI_MULTI(nn.Module):
    ...
    def _model_scores(self, r_src, r_trg, mask_mat, args):
        ...
        if args.hard_clamping is not None:
            ...
            raw_scores = torch.clamp(raw_scores, min=args.hard_clamping[0], max=args.hard_clamping[1]) 
        ...
        if args.loss == "ours":
            ...
            pos_scores = ((mask_pos * -F.softplus(-raw_scores)).sum(dim=(1, 2))) / n_locs
            neg_scores = ((mask_neg * F.softplus(raw_scores)).sum(dim=(1, 2))) / ((n_batch - 1) * n_locs)
    ...
    def _loss_g2l(self, r_src, r_trg, mask_mat, args):
        # compute the scores for these features
        scores, raw_scores, lgt_reg = \
            self._model_scores(r_src, r_trg, mask_mat, args)
        loss_g2l = -scores.mean()
        return loss_g2l, lgt_reg

    def forward(self, r1_src_1, r5_src_1, r1_src_2, r5_src_2,
                r5_trg_1, r7_trg_1, r5_trg_2, r7_trg_2, mask_mat, args, mode):
        ...
        if mode == 'train':
            ...
            if mask_mat.sum().item() < 1e-1:
                ...
                loss_1t5_1, lgt_1t5_1 = self._loss_g2l(r1_src_1, r5_trg_2[0], mask_mat, args)
                loss_1t5_2, lgt_1t5_2 = self._loss_g2l(r1_src_2, r5_trg_1[0], mask_mat, args)
                # compute costs for 1->7 prediction
                loss_1t7_1, lgt_1t7_1 = self._loss_g2l(r1_src_1, r7_trg_2[0], mask_mat, args)
                loss_1t7_2, lgt_1t7_2 = self._loss_g2l(r1_src_2, r7_trg_1[0], mask_mat, args)
                # compute costs for 5->5 prediction
                loss_5t5_1, lgt_5t5_1 = self._loss_g2l(r5_src_1, r5_trg_2[0], mask_mat, args)
                loss_5t5_2, lgt_5t5_2 = self._loss_g2l(r5_src_2, r5_trg_1[0], mask_mat, args)
            ...
            nce_scores, raw_scores, lgt_reg = \
                self._model_scores(r1_src_1, r7_trg_2[0], mask_mat, args)
...
class LossMultiNCE(nn.Module):
    ...
    def forward(self, r1_x1, r5_x1, r7_x1, r1_x2, r5_x2, r7_x2, args):
        '''
